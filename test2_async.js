...
{// TEST 2.1
    ...
    async function sleepOneByOneAsyncAwaited() {
        // Писать код здесь
        const result1 = await sleep500();
        const result2 = await sleep200();
        const result3 = await sleep100();
...
{// TEST 2.3
    ...
    function sleepOneByOne() {
        ...
        return orderedSleeps.reduce((promise, sleep) => {
            return promise.then(() => {
...
{// TEST 2.4
    ...
    async function sleepParallel() {
        // Писать код здесь
        const [result1, result2, result3] = await Promise.all([sleep100(), sleep200(), sleep500()]);
        return result1 + result2 + result3;
    }
